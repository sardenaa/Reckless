      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 pb-8 border-b border-white/5">
        <div className="flex flex-col">
          <div className="flex items-center gap-4 mb-4">
             ...
             <div className="flex flex-col">
               <h1 className="text-6xl font-display italic font-normal uppercase tracking-tight text-white leading-none">THEATER OF OPERATIONS</h1>
               <span className="text-sm font-marker text-(--accent) -rotate-1 ml-2">Monitor active deployments</span>
             </div>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex flex-col gap-2">
            <span className="text-[9px] font-black uppercase text-gray-600 tracking-widest px-1">Time Horizon</span>
          ...
          <div className="flex flex-col gap-2">
            <span className="text-[9px] font-black uppercase text-gray-600 tracking-widest px-1">Operation Type</span>
...
function FilterButton({ active, onClick, label }: any) {
  return (
    <button 
      onClick={onClick}
      className={clsx(
        "px-8 py-4 text-xl font-display uppercase tracking-widest transition-all relative overflow-hidden group italic transform -skew-x-12",
        active 
          ? "bg-white text-black" 
    ...
    >
      <Link to={`/events/${event.id}`}>
        <div className="absolute top-0 right-0 p-4 z-20">
           {isActive ? (
             <div className="bg-(--accent) text-black px-4 py-1 font-display italic text-lg animate-pulse -rotate-3">MISSION ACTIVE</div>
           ) : status === 'completed' ? (
             <div className="bg-emerald-500 text-black px-4 py-1 font-display italic text-lg -rotate-3">MISSION PASSED</div>
           ) : status === 'cancelled' ? (
             <div className="bg-red-600 text-white px-4 py-1 font-display italic text-lg -rotate-3">MISSION FAILED</div>
           ) : (
             <div className="bg-white text-black px-4 py-1 font-display italic text-lg -rotate-3">SCHEDULED</div>
           )}
        </div>

        <div className="p-8 flex flex-col gap-6 pt-16">
          <div>
            <h3 className="text-4xl font-display italic font-normal uppercase mb-2 leading-none group-hover:text-(--accent) transition-colors text-white">{event.title}</h3>
            <span className="text-sm font-marker text-(--accent-secondary) opacity-80">{event.type}</span>
          </div>

          <div className="bg-black/40 border border-white/5 p-6 relative group-hover:bg-white/[0.02] transition-all">
             <p className="text-sm text-gray-400 italic leading-relaxed uppercase tracking-tight line-clamp-3">
              {event.description || "MISSION PARAMETERS CLASSIFIED. CHECK UPLINK FOR INTEL."}
            </p>
          </div>

          <div className="grid grid-cols-2 gap-6 pb-6 border-b border-white/10">
             <div className="flex flex-col">
                <span className="text-[10px] font-black uppercase text-white/30 tracking-widest mb-1">DEPARTURE</span>
                <span className="text-xl font-display italic text-white">{new Date(event.startTime).toLocaleDateString()}</span>
             </div>
             <div className="flex flex-col">
                <span className="text-[10px] font-black uppercase text-white/30 tracking-widest mb-1">LOCATION</span>
                <span className="text-xl font-display italic text-(--accent-secondary)">{event.location || "LOS SANTOS"}</span>
             </div>
          </div>
          ...
          <div className="flex justify-between items-center group">
             <div className="flex items-center gap-3">
                ...
                <span className="text-[10px] font-black uppercase text-white/40 tracking-widest">CITIZENS ENLISTED</span>
             </div>
